            ...
            async.waterfall([
                ...
                function(result, callback) {
                    ...
                    _.each(result.userExchangeGoodsList, function(userExchangeGoods, index) {
                        // list[index] = userExchangeGoods;
                        // list[index].exchangeStatus = result.exchangeStatusMap[userExchangeGoods.status];
                        result.userExchangeGoodsList[index].exchangeStatus = result.exchangeStatusMap[userExchangeGoods.status];
                    });
                    ...
                    userGoodsService.getMapByUserGoodsIdList(_.uniq(userGoodsIdList), function(error, userGoodsMap) {
                        ...
                        _.each(result.userExchangeGoodsList, function(userExchangeGoods, index) {
                            result.userExchangeGoodsList[index].userGoods = userGoodsMap[userExchangeGoods.userGoodsId];
                            result.userExchangeGoodsList[index].exchangeUserGoods = userGoodsMap[userExchangeGoods.exchangeUserGoodsId];
                        });
                        callback(null, result.userExchangeGoodsList);
                    });
                }
            ], callback);
        }
