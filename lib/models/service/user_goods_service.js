 ...
 *     result: ユーザグッズリスト
 */
UserGoodsService.prototype.getMapByUserGoodsIdList = function(userGoodsIdList, callback) {
    try {
    ...
    async.waterfall([
        ...
        // 投稿情報を取得
        function(result, callback) {
            userGoodsDao.getListByIdList(userGoodsIdList, function(error, userGoodsList) {
                if (error) {
                    callback(error);
                    return;
                }
        ...
        function(userGoodsList, callback) {
            ...
            userDao.getMapByIdList(userIdList, function(error, userMap) {
                ...
                callback(null, _.indexBy(userGoodsList, 'id') || null);
            });
        }
    ], callback);
}
